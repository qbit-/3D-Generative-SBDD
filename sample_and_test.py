...
from models.sample import sample_init, sample_refine, cluster_and_select_best, get_next_step
from models.sample_grid import grid_refine, get_grids_batch
from utils.transforms import (FeaturizeLigandAtom, FeaturizeProteinAtom, ContrastiveSample,
                              FeaturizeLigandBond, Compose, LigandCountNeighbors, LigandMaskAll, ATOM_FAMILIES)
from utils.misc import BlackHole, load_config, seed_all, get_new_log_dir, get_logger
...
from utils.chem import filter_rd_mol
...
def prepare_single_data(pocket_fn, ligand_fn, transform):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pocket_fn', type=str)
    parser.add_argument('--ligand_fn', type=str)
    ...
    # Logging
    log_dir = get_new_log_dir(args.outdir, prefix='%s' % (config_name))
    ...
    data = prepare_single_data(args.pocket_fn, args.ligand_fn, transform = transform)
    ...
    writer = Chem.SDWriter(os.path.join(log_dir, 'samples_all.sdf'))
    for data_finished in pool.finished:
